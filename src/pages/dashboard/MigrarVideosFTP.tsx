...
import { ChevronLeft, Server, Upload, Eye, EyeOff, AlertCircle, CheckCircle, Download, Folder, Play, Trash2, FolderOpen, Video, RefreshCw, X, Clock, HardDrive, Wifi, WifiOff, Loader } from 'lucide-react';
...
const MigrarVideosFTP: React.FC = () => {
  ...
  const startMigration = async () => {
    ...
    if (!selectedFolder || selectedFolder === '') {
      toast.error('Selecione uma pasta de destino antes de iniciar a migração');
      return;
    }

    // Verificar se a pasta selecionada existe
    const folderExists = folders.find(f => f.id.toString() === selectedFolder);
    if (!folderExists) {
      toast.error('Pasta de destino não encontrada. Por favor, recarregue a página e tente novamente.');
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        const errorMessage = result.error || 'Erro durante a migração';
        toast.error(errorMessage);
        setIsMigrating(false);
        setShowMigrationModal(false);
      }
    } catch (error: unknown) {
      let errorMessage = 'Erro durante a migração';
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label htmlFor="pasta-destino" className="block text-sm font-medium text-gray-700 mb-2">
                    Pasta de Destino * {!selectedFolder && <span className="text-red-600 text-xs">(Obrigatório)</span>}
                  </label>
                  <select
                    id="pasta-destino"
                    ...
                    className={`w-full px-4 py-2 border rounded-md focus:ring-primary-500 focus:border-primary-500 ${
                      !selectedFolder ? 'border-red-300 bg-red-50' : 'border-gray-300'
                    }`}
                  >
                  ...
                  {!selectedFolder && (
                    <p className="text-xs text-red-600 mt-1">
                      Você precisa selecionar uma pasta para continuar
                    </p>
                  )}
                  {folders.length === 0 && (
                    <p className="text-xs text-yellow-600 mt-1">
                      Nenhuma pasta encontrada. Crie uma pasta primeiro.
                    </p>
                  )}
                </div>
