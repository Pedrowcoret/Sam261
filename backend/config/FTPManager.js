...
class FTPManager {
    ...
    async migrateFiles(userId, files, destinationFolder) {
        try {
            ...
            const [folderRows] = await db.execute(
                'SELECT nome_sanitizado, servidor_id, espaco, espaco_usado FROM folders WHERE id = ? AND user_id = ?',
            ...
            // Buscar espaço total do usuário
            ...
            const userTotalSpace = userData.espaco || 0;
            const espacoUsado = folderData.espaco_usado || 0;
            const availableSpace = userTotalSpace - espacoUsado;
    ...
    async getUserLogin(userId) {
        try {
            // Tentar buscar na tabela streamings primeiro
            const [userRows] = await db.execute(
            ...
            // Se não encontrar, tentar na tabela revenda
            const [revendaRows] = await db.execute(
                'SELECT usuario, email FROM revenda WHERE codigo = ? LIMIT 1',
                [userId]
            );

            if (revendaRows.length > 0 && revendaRows[0].usuario) {
                return revendaRows[0].usuario;
            } else if (revendaRows.length > 0 && revendaRows[0].email) {
                return revendaRows[0].email.split('@')[0];
            }
